...
// Flash message
// ── CONFIRMER OU REFUSER UN DÉPÔT ──
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_depot'])) {
    $cotisation_id = (int) $_POST['cotisation_id'];
    $action        = $_POST['action_depot']; // 'confirmer' | 'refuser'
    $tid           = (int) $_POST['tontine_id'];

    // Vérifier que la cotisation appartient bien à une tontine du représentant
    $check = $pdo->prepare("
        SELECT c.*, t.representant_id
        FROM cotisations c
        JOIN tontines t ON c.tontine_id = t.id
        WHERE c.id = ? AND t.representant_id = ? AND c.statut = 'en_attente'
    ");
    $check->execute([$cotisation_id, $user_id]);
    $cotisation = $check->fetch();

    if ($cotisation) {
        if ($action === 'confirmer') {
            // Marquer payée
            $pdo->prepare("UPDATE cotisations SET statut = 'payee' WHERE id = ?")
                ->execute([$cotisation_id]);
            // Créditer le solde du membre
            $pdo->prepare("
                UPDATE membres_tontines SET solde = solde + ?
                WHERE tontine_id = ? AND membre_id = ?
            ")->execute([$cotisation['montant'], $cotisation['tontine_id'], $cotisation['membre_id']]);

            log_action('Dépôt confirmé', "Cotisation $cotisation_id – {$cotisation['montant']} FCFA confirmée");
            $_SESSION['flash'] = ['type' => 'success', 'msg' => " Dépôt de " . number_format($cotisation['montant'], 0, ',', ' ') . " FCFA confirmé. Solde membre mis à jour."];
        } else {
            // Refuser
            $pdo->prepare("UPDATE cotisations SET statut = 'en_retard' WHERE id = ?")
                ->execute([$cotisation_id]);
            log_action('Dépôt refusé', "Cotisation $cotisation_id refusée");
            $_SESSION['flash'] = ['type' => 'danger', 'msg' => " Dépôt refusé."];
        }
    }
    header("Location: dashboard_representant.php?tontine_id=$tid");
    exit();
}
...
if ($tontine) {
    ...
    $s = $pdo->prepare("SELECT COUNT(*) FROM membres_tontines WHERE tontine_id = ? AND statut = 'actif'");
    $s->execute([$tid]);
    $stats['nb_membres'] = $s->fetchColumn();
    $s = $pdo->prepare("SELECT COUNT(*) FROM cotisations WHERE tontine_id = ? AND DATE(date_paiement)=CURDATE() AND statut='payee'");
    $s->execute([$tid]);
    $stats['cotisations_jour'] = $s->fetchColumn();
    $s = $pdo->prepare("SELECT COUNT(*) FROM cotisations WHERE tontine_id = ? AND statut='en_retard'");
    $s->execute([$tid]);
    $stats['cotisations_retard'] = $s->fetchColumn();
    $s = $pdo->prepare("SELECT COUNT(*) FROM prets WHERE tontine_id = ? AND statut='en_attente'");
    $s->execute([$tid]);
    $stats['prets_en_attente'] = $s->fetchColumn();
    $s = $pdo->prepare("SELECT COALESCE(SUM(montant),0) FROM cotisations WHERE tontine_id = ? AND MONTH(date_paiement)=MONTH(CURDATE()) AND YEAR(date_paiement)=YEAR(CURDATE()) AND statut='payee'");
    $s->execute([$tid]);
    $stats['cotisations_mois'] = $s->fetchColumn();
    $s = $pdo->prepare("SELECT COALESCE(SUM(montant),0) FROM cotisations WHERE tontine_id = ? AND statut='payee'");
    $s->execute([$tid]);
    $stats['total_collecte'] = $s->fetchColumn();
    ...
    $s->execute([$tid]);
    $cotisations_recentes = $s->fetchAll();
...
<head>
    ...
    <style>
        ...
        .mod-card:hover {
            ...
            box-shadow: 0 8px 24px rgba(24, 57, 43, .2);
        }
          ...
          box-shadow:0 4px 16px rgba(14,165,233,.25);">
                ...
                <div class="kpi-grid">
                    <div class="kpi">
                        ...
                        <div>
                            <div class="kpi-val"><?= count($tontines_rep) ?></div>
                            <div class="kpi-lbl">Mes tontines</div>
                        </div>
                    </div>
                    <div class="kpi">
                        <div class="kpi-icon" style="background:#dbeafe;color:#1d4ed8;"><i class="bi bi-people-fill"></i></div>
                        <div>
                            <div class="kpi-val"><?= $total_membres_global ?></div>
                            <div class="kpi-lbl">Membres total</div>
                        </div>
                    </div>
                    <div class="kpi">
                        <div class="kpi-icon" style="background:#fef9c3;color:#854d0e;"><i class="bi bi-cash-coin"></i></div>
                        <div>
                            <div class="kpi-val"><?= number_format($total_collecte_global / 1000, 0) ?>K</div>
                            <div class="kpi-lbl">FCFA collectés</div>
                        </div>
                    </div>
                </div>
                    ...
                    <?php if ($tontine): ?>
                        ...
                        <div class="kpi-grid" style="grid-template-columns:repeat(auto-fill,minmax(140px,1fr));">
                            <div class="kpi">
                                <div class="kpi-icon" style="background:#dbeafe;color:#1d4ed8;"><i class="bi bi-people-fill"></i></div>
                                <div>
                                    <div class="kpi-val"><?= $stats['nb_membres'] ?></div>
                                    <div class="kpi-lbl">Membres actifs</div>
                                </div>
                            </div>
                            <div class="kpi">
                                <div class="kpi-icon" style="background:#d1fae5;color:#065f46;"><i class="bi bi-cash-stack"></i></div>
                                <div>
                                    <div class="kpi-val"><?= number_format($stats['cotisations_mois'] / 1000, 0) ?>K</div>
                                    <div class="kpi-lbl">Ce mois (FCFA)</div>
                                </div>
                            </div>
                            <div class="kpi">
                                <div class="kpi-icon" style="background:#fee2e2;color:#991b1b;"><i class="bi bi-exclamation-triangle-fill"></i></div>
                                <div>
                                    <div class="kpi-val"><?= $stats['cotisations_retard'] ?></div>
                            ...
                            <div class="kpi">
                                ...
                                <div>
                                    <div class="kpi-val"><?= $stats['prets_en_attente'] ?></div>
                                    <div class="kpi-lbl">Prêts à traiter</div>
                                </div>
                            </div>
                            <div class="kpi">
                                <div class="kpi-icon" style="background:#ede9fe;color:#5b21b6;"><i class="bi bi-check2-all"></i></div>
                                <div>
                                    <div class="kpi-val"><?= $stats['cotisations_jour'] ?></div>
                                    <div class="kpi-lbl">Payés aujourd'hui</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- ══ DÉPÔTS EN ATTENTE DE CONFIRMATION ══ -->
                        <?php if ($nb_depots_attente > 0): ?>
                            <div class="s-card" style="border-left:4px solid #f59e0b;">
                                <div class="s-card-hdr">
                                    <h3>
                                        ...
                                        Dépôts à confirmer
                                        <span style="background:#fef3c7;color:#92400e;border-radius:20px;padding:2px 10px;font-size:.72rem;font-weight:700;margin-left:8px;">
                                            <?= $nb_depots_attente ?> en attente
                                        </span>
                                    </h3>
                                </div>
                                <div style="padding:16px;display:flex;flex-direction:column;gap:10px;">
                                    <?php foreach ($depots_attente as $d): ?>
                                    ...
                                    font-weight:800;font-size:.9rem;flex-shrink:0;">
                                                ...
                                                <div>
                                                    ...
                                                    <div style="font-size:.78rem;color:#64748b;">
                                                        <?= $d['telephone'] ? htmlspecialchars($d['telephone']) : '—' ?> ·
                                                        <?= date('d/m/Y', strtotime($d['date_paiement'])) ?>
                                                    </div>
                                                    <div style="font-size:.78rem;color:#94a3b8;margin-top:2px;">
                                                        Réf: <?= $d['reference_paiement'] ? htmlspecialchars(substr($d['reference_paiement'], 0, 20)) : '—' ?>
                                                        <?php if ($d['mode_paiement'] === 'mtn_momo'): ?>
                                                            · <span style="color:#92400e;"> MTN MoMo</span>
                                                        <?php elseif ($d['mode_paiement'] === 'orange_money'): ?>
                                                            · <span style="color:#9a3412;"> Orange Money</span>
                                                        <?php endif; ?>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                                                <div style="font-family:'Cormorant Garamond',serif;font-size:1.3rem;font-weight:800;color:#18392B;">
                                                    <?= number_format($d['montant'], 0, ',', ' ') ?> FCFA
                                                </div>

                                                <!-- Confirmer -->
                                                <form method="POST" style="display:inline;">
                                                    <input type="hidden" name="action_depot" value="confirmer">
                                                    <input type="hidden" name="cotisation_id" value="<?= $d['id'] ?>">
                                                    <input type="hidden" name="tontine_id" value="<?= $tontine['id'] ?>">
                                                    <button type="submit"
                                                        onclick="return confirm('Confirmer le dépôt de <?= number_format($d['montant'], 0, ',', ' ') ?> FCFA de <?= htmlspecialchars(addslashes($d['prenom'] . ' ' . $d['nom'])) ?> ?')"
                                           ...
                                           border:none;cursor:pointer;transition:opacity .2s;">
                                                ...
                                                <!-- Refuser -->
                                                <form method="POST" style="display:inline;">
                                                    <input type="hidden" name="action_depot" value="refuser">
                                                    <input type="hidden" name="cotisation_id" value="<?= $d['id'] ?>">
                                                    <input type="hidden" name="tontine_id" value="<?= $tontine['id'] ?>">
                                                    <button type="submit"
                                                        onclick="return confirm('Refuser ce dépôt ?')"
                                                        style="display:inline-flex;align-items:center;gap:5px;
                                           padding:7px 14px;border-radius:8px;
                        ...
                        <div style="display:grid;grid-template-columns:2fr 1fr;gap:18px;margin-bottom:20px;">
                            ...
                            <div style="display:flex;flex-direction:column;gap:14px;">
                                <div class="s-card">
                                    <div class="s-card-hdr">
                                        <h3><i class="bi bi-lightning-charge-fill me-2 text-warning"></i>Actions rapides</h3>
                                    </div>
